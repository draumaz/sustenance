...
import io.github.draumaz.sustenance.data.MetricKind
...
private fun MetricWidgetContent() {
    ...
    Column(
        ...
        when {
            !granted -> Text(
                text = context.getString(io.github.draumaz.sustenance.R.string.tap_to_connect),
                style = TextStyle(color = GlanceTheme.colors.onSurfaceVariant, fontSize = 13.sp),
            )
            points.size < 2 -> {
                Text(value, style = TextStyle(color = GlanceTheme.colors.onSurface, fontWeight = FontWeight.Bold, fontSize = 30.sp))
                Text(context.getString(io.github.draumaz.sustenance.R.string.no_recent_data), style = TextStyle(color = GlanceTheme.colors.onSurfaceVariant, fontSize = 12.sp))
            }
            else -> {
                Text(value, style = TextStyle(color = GlanceTheme.colors.onSurface, fontWeight = FontWeight.Bold, fontSize = 30.sp))
